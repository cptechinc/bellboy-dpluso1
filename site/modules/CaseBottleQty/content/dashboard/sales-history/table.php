...
<table class="table table-striped table-bordered table-condensed order-listing-table">
	<thead>
		<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/sales-history/thead-rows.php'; ?>
	</thead>
	<tbody>
		...
		<?php foreach($orderpanel->orders as $order) : ?>
			...
			<?php if ($order->ordernumber == $input->get->text('ordn')) : ?>
				<?php if ($input->get->show == 'documents' && (!$input->get('item-documents'))) : ?>
					<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/sales-history/documents-rows.php'; ?>
				<?php endif; ?>

				<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/sales-history/detail-rows.php'; ?>

				<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/sales-history/totals-rows.php'; ?>

				<?php if ($input->get->text('show') == 'tracking') : ?>
					<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/sales-history/tracking-rows.php'; ?>
				<?php endif; ?>

				<?php if ($order->error == 'Y') : ?>
					 <tr class="detail bg-danger" >
						<td colspan="2" class="text-center"><b class="text-danger">Error:</b></td>
						<td colspan="3"><?= $order->errormsg; ?></td> <td></td> <td></td>
