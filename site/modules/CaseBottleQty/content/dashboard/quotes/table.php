<?php use Dplus\ProcessWire\DplusWire; ?>
<table class="table table-striped table-bordered table-condensed" id="quotes-table">
	<thead>
		<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/quotes/thead-rows.php'; ?>
	</thead>
	<tbody>
			...
			<?php if ($quote->quotnbr == $input->get->text('qnbr')) : ?>
				<?php if ($quote->error == 'Y') : ?>
					<tr class="detail bg-danger" >
						<td></td>
						<td></td>
						<td colspan="4"><b>Error: </b><?= $quote->errormsg; ?></td>
						<td></td>
						<td></td>
						<td></td>
						<td></td>
				...
				<?php endif; ?>
				<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/quotes/detail-rows.php'; ?>
				<?php include $config->paths->siteModules.'CaseBottleQty/content/dashboard/quotes/totals-rows.php'; ?>
