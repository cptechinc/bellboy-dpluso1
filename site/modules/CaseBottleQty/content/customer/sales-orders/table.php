<table class="table table-striped table-bordered table-condensed" id="orders-table">
	<thead>
		<?php include $config->paths->siteModules.'CaseBottleQty/content/customer/sales-orders/thead-rows.php'; ?>
	</thead>
	<tbody>
		<?php if (isset($input->get->ordn)) : ?>
			<?php if ($orderpanel->count == 0 && $input->get->text('ordn') == '') : ?>
				<tr> <td colspan="13" class="text-center">No Orders found! Try using a date range to find the order(s) you are looking for.</td> </tr>
			<?php endif; ?>
		<?php endif; ?>
		...
		<?php foreach ($orderpanel->orders as $order) : ?>
			<tr class="<?= $order->ordernumber == $input->get->text('ordn') ? 'selected' : ''; ?>" id="<?= $order->ordernumber; ?>">
				...
				<td align="right">$ <?= $page->stringerbell->format_money($order->ordertotal); ?></td> <td align="right" ><?= $order->order_date; ?></td>
			...
			<?php if ($order->ordernumber == $input->get->text('ordn')) : ?>
				<?php if ($input->get->text('show') == 'documents' && (!$input->get('item-documents'))) : ?>
					<?php include $config->paths->siteModules.'CaseBottleQty/content/customer/sales-orders/documents-rows.php'; ?>
				<?php endif; ?>

				<?php include $config->paths->siteModules.'CaseBottleQty/content/customer/sales-orders/detail-rows.php'; ?>

				<?php include $config->paths->siteModules.'CaseBottleQty/content/customer/sales-orders/total-rows.php'; ?>

				<?php if ($input->get->text('show') == 'tracking') : ?>
					<?php include $config->paths->siteModules.'CaseBottleQty/content/customere/sales-orders/tracking-rows.php'; ?>
				<?php endif; ?>
