			...
			<?php
				if ($modules->isInstalled('CaseBottleQty')) {
					if ($appconfig->allow_changeprice) {
						include $config->paths->siteModules.'CaseBottleQty/content/edit/pricing/quotes/tables/price-edit-table.php';
					} else {
						include $config->paths->siteModules.'CaseBottleQty/content/edit/pricing/quotes/tables/price-static-table.php';
					}
				} else {
					if ($appconfig->allow_changeprice) {
