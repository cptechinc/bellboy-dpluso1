			...
			<?php
				if ($modules->isInstalled('CaseBottleQty')) {
					if ($appconfig->allow_changeprice) {
						include $config->paths->siteModules.'CaseBottleQty/content/edit/pricing/sales-orders/tables/price-edit-table.php';
					} else {
						include $config->paths->siteModules.'CaseBottleQty/content/edit/pricing/sales-orders/tables/price-static-table.php';
					}
				} else {
					if ($appconfig->allow_changeprice) {
