	...
	switch ($page->parent->name) { //$page->name is what we are editing
		case 'order':
			...
			if ($modules->isInstalled('CaseBottleQty')) {
				$page->body = $config->paths->siteModules.'CaseBottleQty/content/confirm/sales-order.php';
			} else {
		...
		case 'quote':
			...
			if ($modules->isInstalled('CaseBottleQty')) {
				$page->body = $config->paths->siteModules.'CaseBottleQty/content/confirm/quote.php';
			} else {
